...
pub fn handle_command(
    ...
    let key = ConversationKey::new(PLATFORM_ID, channel_id.to_string());
...
mod tests {
    ...
    fn disallowed_user_is_ignored() {
        ...
        let outcome = handle_command(&runtime, 1, Some(7), "hi", &config).unwrap();
    ...
    fn empty_message_is_ignored() {
        let runtime = test_runtime(tempdir().unwrap().path().to_path_buf());
        let outcome = handle_command(&runtime, 1, Some(1), "   ", &open_config()).unwrap();
        assert_eq!(outcome, CommandOutcome::Ignored);
    }
    ...
    fn start_returns_welcome_without_touching_the_agent() {
        ...
        let outcome = handle_command(&runtime, 1, Some(1), "/start", &config).unwrap();
    ...
    fn help_command_is_handled_locally() {
        let runtime = test_runtime(tempdir().unwrap().path().to_path_buf());
        let outcome = handle_command(&runtime, 1, Some(1), "/help", &open_config()).unwrap();
        match outcome {
    ...
    fn new_command_detaches_the_existing_session() {
        let runtime = test_runtime(tempdir().unwrap().path().to_path_buf());
        let key = ConversationKey::new(PLATFORM_ID, "555");
        ...
        let outcome = handle_command(&runtime, 555, Some(1), "/new", &open_config()).unwrap();
    ...
    fn allowed_user_passes_filter() {
        ...
        let outcome = handle_command(&runtime, 1, Some(42), "/help", &config).unwrap();
        assert!(matches!(outcome, CommandOutcome::Reply(_)));
    }
}
